"""Export daily management input to Daily Input tab."""
...
EXPORT_COLS = [
    ...
    "cogs", "logistics", "partner_services", "fbo_services",
    "gross_profit", "margin", "plan_vp", "deviation",
...
def _load_from_db() -> pd.DataFrame | None:
    try:
        ...
        rows = execute_query(
            ...
            "COALESCE(partner_services, 0) as partner_services, "
            "COALESCE(fbo_services, 0) as fbo_services, "
            "COALESCE(advertising, 0) as advertising "
            "FROM finance ORDER BY date DESC LIMIT 31"
        )
    except Exception as e:
        logger.warning("DB unavailable for Daily Input: %s", e)
        return None
    ...
    df["cogs"] = 0
    df["gross_profit"] = (
        df["revenue"].astype(float) - df["commission"].astype(float) - df["logistics"].astype(float)
        - df["advertising"].astype(float) - df["partner_services"].astype(float) - df["fbo_services"].astype(float)
    ).round(2)
    df["margin"] = (
...
def _load_from_files() -> pd.DataFrame | None:
    try:
        ...
        logger.warning("File source unavailable for Daily Input: %s", e)
        return None
...
def export_daily_input(ws: gspread.Worksheet, *, use_files: bool = False) -> int:
