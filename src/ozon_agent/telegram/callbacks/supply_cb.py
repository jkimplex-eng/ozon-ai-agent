...
async def _show_supply(query: Any, cluster_name: str | None = None) -> None:
    ...
    if not proposal:
        ...
        await query.edit_message_text(
            text,
            reply_markup=supply_keyboard(cluster_buttons=cluster_buttons, selected_cluster=cluster_name),
        )
        return
...
async def _show_timeslots(query: Any, proposal_id: str) -> None:
    ...
    if not proposal or proposal.proposal_id != proposal_id or not proposal.draft_id:
        await query.edit_message_text(
            ...
            reply_markup=supply_keyboard(proposal_id, cluster_buttons=_cluster_buttons_for_supply()),
        )
        return
    ...
    await query.edit_message_text(
        text,
        reply_markup=supply_keyboard(proposal.proposal_id, cluster_buttons=_cluster_buttons_for_supply()),
    )
